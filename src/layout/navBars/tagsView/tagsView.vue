<template>
	<div class="layout-navbars-tagsview" :class="{ 'layout-navbars-tagsview-shadow': getThemeConfig.layout === 'classic' }">
		<el-scrollbar ref="scrollbarRef" @wheel.native.prevent="onHandleScroll">
			<ul class="layout-navbars-tagsview-ul" :class="setTagsStyle" ref="tagsUlRef">
				<li
					...
					"
				>
					<em class="iconfont icon-webicon318 layout-navbars-tagsview-ul-li-iconfont" v-if="isActive(v)"></em>
					...
					<SvgIcon
						...
						v-if="!v.meta.isAffix && state.tagsViewList.length > 1"
						@click.stop="closeCurrentTagsView(getThemeConfig.isShareTagsView ? v.path : v.url)"
					/>
				</li>
			</ul>
		...
		<Contextmenu
			...
			ref="contextmenuRef"
			:tagViewList="state.tagsViewList"
...
const addBrowserSetSession = (tagsViewList) => {
	...
	proxy.mittBus.emit('onRefreshTagSession', tagsViewList);
};
...
const addTagsView = (path, to) => {
	...
	nextTick(async () => {
		...
		// console.log(`output->state.tagsViewRoutesList`,state.tagsViewRoutesList)
		...
		// console.log('get limitTagNumber：', getThemeConfig.value.limitTagNumber);

		if (getThemeConfig.value.limitTagNumber && state.tagsViewList.length > getThemeConfig.value.limitTagNumber) {
			const closeTagName = state.tagsViewList.filter((item, index) => state.tagsViewList.length - index > 10).map((item) => item.name);
			const index = 0 - getThemeConfig.value.limitTagNumber;
			state.tagsViewList = state.tagsViewList.slice(index); // 配置
			if (closeTagName && closeTagName.length) {
				proxy.mittBus.emit('onTagsViewCloseRouterView', closeTagName);
			}
		}
...
const closeCurrentTagsView = (path) => {
	const closeTagName = []; // 关闭的tag集合
	state.tagsViewList.map((v, k, arr) => {
		if (!v.meta.isAffix) {
			if (getThemeConfig.value.isShareTagsView ? v.path === path : v.url === path) {
				// add by hfcai on 2022/08/17
				const t = state.tagsViewList[k];
				if (t && t.meta && t.meta.isKeepAlive) {
					// 当改tag为缓存路由时需要通知添加去除通知
					closeTagName.push(t.name);
				}
				// 控制路由缓存事件
...
const closeOtherTagsView = (path) => {
	const closeTagName = state.tagsViewList.map((item) => item.name).filter((item) => item.name !== route.name); // 关闭的tag集合
	state.tagsViewList = [];
...
const closeAllTagsView = () => {
	const closeTagName = state.tagsViewList.map((item) => item.name);
	state.tagsViewList = [];
	state.tagsViewRoutesList.map((v) => {
		if (v.meta.isAffix && !v.meta.isHide) {
			state.tagsViewList.push({ ...v });
...
const onCurrentContextmenuClick = async (item) => {
	// console.log('get onCurrentContextmenuClick item:',item)
	const cParams = item.meta.isDynamic ? item.params : item.query;

	// console.log(getCurrentRouteItem(item.path, cParams));
