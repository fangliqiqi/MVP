  ...
  <div class="edit-staff-page">
    <div class="content">
      <el-form ref="formRef" :model="state.formData" :rules="state.rules"   size="default" label-width="160px">
        <div v-for="(data,key) in PREORDER_HEADER_QUERY" :key="key">
          <div class="title">
            <span>{{key}}</span>
          </div>
...
<script setup name="detailQuery">

import{ PREORDER_HEADER_QUERY }from'/@/utils/header';
...
const loadDicts = async()=>{
  try {
    ...
    for(let p in PREORDER_HEADER_QUERY){
      for(let i of PREORDER_HEADER_QUERY[p]){
        if(i.dict){
...
<style lang="scss" scoped  src="./css/detailQuery.scss">
</style>
<style>
  .tooltip1_6{
    white-space: nowrap;
