		...
		<el-tabs v-model="state.activeInfo" type="card" class="attach-tabs" style="margin-top: 16px">
			...
			<el-tab-pane v-for="item in state.config" :key="`attach_tab_${item.name}`" :label="item.label" :name="item.name">
				...
				<template v-else-if="item.name === 'social'">
					<!--社保信息-->
					<div style="text-align: center">暂时还没设计，都空着~</div>
				</template>
...
import { getSubjectChangeLog } from '/@/api/staff';
...
const requestContractList = async (params) => {
	try {
		const res = await getContractSearchList(Object.assign({}, params, { empIdcard: props.info && props.info.empIdcard }));
		if (res && res.code == 200 && res.data) {
			return {
				current: res.data.current,
...
const getValue = (item) => {
	...
	switch (item) {
		...
		case 'businessPrimaryType':
			val = formatBusinessType(
				getDictValue('customer_business_parent', props.info.businessPrimaryType, state.DICTS),
				getDictValue('customer_business_type', props.info.businessSecondType, state.DICTS),
				getDictValue('customer_business_sub_type', props.info.businessThirdType, state.DICTS)
			);
			break;
		case 'projectStatus':
			val =
...
<style lang="scss" scoped src="./css/staffProject.scss">
