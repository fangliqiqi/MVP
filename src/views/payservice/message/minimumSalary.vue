...
<script setup name="最低工资提醒">
import ProTable from '/@/components/ProTable/ProTable.vue';
import ProButtons from '/@/components/ProButtons/index.vue';
import feedBackVue from './component/feedback.vue'
import { ElMessage, ElMessageBox } from 'element-plus';
import { getminSalaryList,exportMinSalary } from '/@/api/payservice/message';
...
const handleBatchExport = () => {
	...
	exportMinSalary(state.params).then(res=>{
		if(res.code==200){
...
const onCellClick = async (prop, item) => {
	switch (prop) {
		case 'feedback':
			feedbackVueRef.value&&feedbackVueRef.value.openDialog(item,'minSalary');
			break;
	}
};
