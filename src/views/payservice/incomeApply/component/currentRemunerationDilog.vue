...
<script setup name="currentremunerationdilog">
import { deleteBatchRemuneration} from '/@/api/payservice/currentRemuneration';
		...
		formRef.value.validate((valid) => {
			if (valid) {
				isShowLoading.value = true
					// 删除操作
					deleteBatchRemuneration(state.ruleForm)
						.then(
							(res) => {
								if (res && res.code == 200) {
									emits('ok');
