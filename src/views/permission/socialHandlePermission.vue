...
<template>
	...
	<!-- 新增 社保审核权限-->
	<add-social-handle-permission ref="addSocialHandlePermissionRef" @ok="refresh"/>
	</div>
</template>

<script setup name="社保办理权限">
import ProTable from '/@/components/ProTable/ProTable.vue';
import ProButtons from '/@/components/ProButtons/index.vue';
import addSocialHandlePermission from './component/addSocialHandlePermission.vue'
import { ElMessage, ElMessageBox } from 'element-plus';
import { PERMISSION_SOCIAL_HEADER_EXPORT } from '/@/utils/header.js';
import {exportExcelSheet} from '/@/utils/excel';
import moment from 'moment';

import {getTableList,deleteInfo,getExportList,disabledAndNotDisabled} from '/@/api/permission/socialHandlePermission';

const proTableRef = ref(null);
const store = useStore();
const addSocialHandlePermissionRef  = ref(null)
const refresh = () => {
	// 列表刷新
...
// 新增
const handleAdd = () => {
  if (addSocialHandlePermissionRef.value) addSocialHandlePermissionRef.value.openDialog();
};
// 导出
const handleBatchExport = () => {
	...
	if(state.selectionChangeList.length!=0){
			p =Object.assign({},{houseNameType:0},state.params); 
	}else{
			p = Object.assign({},{houseNameType:0},state.params,proTableRef.value.getParams());
	}
			delete p.size;
			delete p.current;
			getExportList(p).then((res) => {
				exportExcelSheet(
					...
					`社保办理权限批量导出${moment().format('YYYYMMDD-HHmmss')}`
				)
...
const request = async (params) => {
	try {
		params = Object.assign({},{houseNameType:0,	authType:1},params)
