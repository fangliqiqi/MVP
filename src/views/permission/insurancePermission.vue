<template>
	<div class="cardinal-config-container">
		...
		<!-- 新增 -->
		<add-insurance-permission-vue ref="addInsurancePermissionRef" @ok="refresh"/>
	</div>
</template>

<script setup name="商险办理权限">
import ProTable from '/@/components/ProTable/ProTable.vue';
import ProButtons from '/@/components/ProButtons/index.vue';
import addInsurancePermissionVue from './component/addInsurancePermission.vue'
import { getDictByItemTypes } from '/@/utils/utils';
import { ElMessage, ElMessageBox } from 'element-plus';
import { getInsurancePerlist,exportInsurancePer,deleteInsurancePer,updateInsurancePer} from '/@/api/permission/insurancePermission';



const proTableRef = ref(null);
const addInsurancePermissionRef = ref(null);
...
// 新增
const handleAdd = () => {
  addInsurancePermissionRef.value&&addInsurancePermissionRef.value.openDialog()
};
...
const handleBatchExport = () => {
	...
	exportInsurancePer(state.params).then(res=>{
		if(res.code==200){
...
const state = reactive({
	...
	btn: [
    {
			...
			permission: ['tinsuranceareares_add'],
		...
		},
		{
			...
			loading:isShowLoading,
			permission: ['tinsuranceareares-export'],
      type:'normal',
	...
	cellBtns: [
		{
			text: '删除',
			prop: 'delete', // 唯一标识
			permission: ['tinsuranceareares_del'],
			// isGray:(row)=>{
		...
		},
    {
			text: '启用',
			prop: 'enable', // 唯一标识
			permission: ['tinsuranceareares_edit'],
			isGray:(row)=>{
				return row.status == '0'
			},
		},
	...
	switch (prop) {
		case 'delete':
			...
			ElMessageBox.confirm(`是否确定删除？`, '提示', {
				...
				.then(async () => {
					try {
						const res = await deleteInsurancePer(item.id);
						if (res && res.code === 200) {
		...
		case 'enable':
			...
			ElMessageBox.confirm(`是否确定启用？`, '提示', {
				...
				.then(async () => {
					try {
						const res = await updateInsurancePer({
							id: item.id,
							status: 0,
						});
						if (res && res.code === 200) {
...
const request = async (params) => {
	try {
		const res = await getInsurancePerlist(params);
