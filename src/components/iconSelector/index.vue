		...
		<el-popover placement="bottom" :width="state.fontIconWidth" v-model:visible="state.fontIconVisible" popper-class="icon-selector-popper">
			<template #reference>
				<el-input
					...
					:placeholder="state.fontIconPlaceholder || '请选择'"
					:clearable="clearable"
					:disabled="disabled"
					:size="size"
					ref="inputWidthRef"
					...
					@focus="onIconFocus"
					@blur="onIconBlur"
					:readonly="!state.fontIconVisible"
				>
					<template #prepend>
						<SvgIcon
						...
						/>
						<em v-else :class="state.fontIconPrefix === '' ? prepend : state.fontIconPrefix" class="font14"></em>
					</template>
				</el-input>
			</template>
...
// 事件
const emit = defineEmits(['update:modelValue', 'get', 'clear', 'nextInput']);
...
const triggerVisible = () => {
	state.fontIconVisible = !state.fontIconVisible;
	if (state.fontIconVisible) {
		setTimeout(() => {
