...
BRANCH_ARRAY=('dev' 'master' 'dev-ins')
FILENAME="yifu-qas-webmvp:1.0.0"
NAMESPACE_ARRAY=('qas-mvp' 'prd-mvp' 'qas-mvp')
ENV_ARRAY=('qas' 'prd' 'qas')
WEB_DEPLOY_DIR="./deploy/qas/k8s-yaml/"
...
rtp_branch_selection()
{
    if [ -z "${BRANCH_SELECT}" ]; then
        ...
        echo "3: 更新推送 ${BRANCH_ARRAY[2]} 分支代码"
        echo "0: 不更新"
        read -p "请输入你的选择 (1, 2, 3 or 0): " BRANCH_SELECT
    fi
    ...
    2)
        ...
        ;;
    3)
        Echo_Green "即将更新 ${BRANCH_ARRAY[2]} 服务"
        rtp_update
        ;;
    0)
        Echo_Green "不更新任何服务"
...
cd ${WEB_DEPLOY_DIR}
kubectl delete -f qas-yifu-app-mvp-webmvp-deployment.yaml
kubectl create -f qas-yifu-app-mvp-webmvp-deployment.yaml
